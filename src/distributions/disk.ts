...
const annulus=(s:number,ds:number,sigma:number,d:number):ChargeSample=>{const dq=sigma*2*Math.PI*s*ds;return{position:{x:s,y:0,z:0},dq,coordinate:s,field:{x:0,y:0,z:K*dq*d/(s*s+d*d)**1.5}};};
