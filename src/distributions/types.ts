...
/** `potential` is k·dq/rᵢ, the scalar the potential problems accumulate in place of the field vector. */
export type ChargeSample={position:Vec;dq:number;field:Vec;potential:number;coordinate:number};
 ...
 * `potential` is the closed-form V at P in volts, for the geometries whose potential problem exists. */
export type Distribution={id:string;field(p:Params):Vec;quadrature(p:Params,n:number):Vec;sample(p:Params,n:number):ChargeSample[];substitutions?:Record<string,string>;total?(p:Params):number;potential?(p:Params):number};
...
export function pointSample(position:Vec,observation:Vec,dq:number,coordinate:number):ChargeSample{
 ...
 const r=Math.hypot(delta.x,delta.y,delta.z),coefficient=K*dq/r**3;
 return{position,dq,coordinate,field:{x:coefficient*delta.x,y:coefficient*delta.y,z:coefficient*delta.z},potential:K*dq/r};
}
