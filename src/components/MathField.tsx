...
type MathFieldElement=HTMLElement&{value:string;setOptions?:(o:Record<string,unknown>)=>void};
...
export type MathFieldProps={value:string;onChange:(latex:string)=>void;label:string;placeholder?:string;invalid?:boolean;id?:string;onEnter?:()=>void};
export function MathField({value,onChange,label,placeholder,invalid,id,onEnter}:MathFieldProps){
 const host=useRef<MathFieldElement|null>(null),latest=useRef(onChange);latest.current=onChange;
 ...
 useEffect(()=>{
  ...
  el.setOptions?.({smartFence:true,smartMode:false,mathVirtualKeyboardPolicy:'auto'});
 ...
 if(!ready)return <input id={id} aria-label={label} aria-invalid={invalid} className={'mathfield-fallback'+(invalid?' invalid':'')} value={value} placeholder={placeholder} spellCheck={false} autoComplete="off" onChange={e=>onChange(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();onEnter?.()}}}/>;
