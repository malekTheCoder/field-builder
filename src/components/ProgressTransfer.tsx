...
export function ProgressTransfer({data,onChange}:{data?:ProgressData;onChange?:(next:ProgressData)=>void}={}){
 ...
 return <><button type="button" className="icon-button no-print" onClick={save} aria-label="Export lesson progress"><FolderOutput size={16}/></button><button type="button" className="icon-button no-print" onClick={()=>input.current?.click()} aria-label="Import lesson progress"><FolderInput size={16}/></button><input ref={input} type="file" accept="application/json,.json" className="sr-only" aria-hidden="true" tabIndex={-1} onChange={e=>{const file=e.target.files?.[0];e.target.value='';if(file)void read(file);}}/><output className="sr-only" aria-live="polite">{status}</output></>;
}
