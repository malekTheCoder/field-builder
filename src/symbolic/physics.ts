...
export function sampleLimit(problem:Problem,p:Params,limit:Limit,t:number){const copy={...p},q=Math.abs(REGISTRY[problem.id].total?.(p)??p.charge*1e-9),R=p.size/2;let actual=0,target=0,label='';switch(limit.mode){case 'far':copy.distance=t*p.size;actual=magnitude(field(problem.id,copy));target=K*q/copy.distance**2;label=`distance / ${problem.id==='bisector'||problem.id==='axial'||problem.id==='endpoint'||problem.id==='ramp'?'L':'2R'} = ${t.toFixed(1)}`;break;
